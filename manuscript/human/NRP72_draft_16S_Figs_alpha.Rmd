...
alpha_plot_noleg$data %>% 
  ...
  group_by(Model2, variable) %>% 
  arrange(p) %>% 
  select(-".y.", -p.adj, -p.format, -p.signif) %>% 
  rstatix::adjust_pvalue(method = "fdr",p.col = "p", output.col = "p.adj") %>% 
  rstatix::add_significance(p.col = "p.adj") -> p_tmp
...
```{r}
alpha_plot_noleg$data %>% 
  filter(Day_of_Treatment >= 3 & Day_of_Treatment <= 6) %>% 
  # group_by(Model2) %>% 
  # group_by(Model2, variable) %>%
  # rstatix::wilcox_test(log10_ratio ~ Treatment_A,
  #                      # comparisons = list(c("Inulin_3", "Control")),
...
setNames(vector("list", 
                length(unique(alpha_plot_noleg$data$Reactor_Treatment_Dose))), 
         unique(alpha_plot_noleg$data$Reactor_Treatment_Dose)) -> lis
```
...
alpha_plot_noleg$data %>%
  dplyr::filter(Reactor_Treatment_Dose == UQ(grp)) %>%
  group_by(Model2, variable) %>% 
  # rstatix::wilcox_test(value ~ period,
  #                       data = . , p.adjust.method = "none")-> lis[[grp]]  #,
  ggpubr::compare_means(value ~ period,
                        data = .,
                        method = "wilcox.test",
                        group.by = c("Model2", "variable"),
                        p.adjust.method = "none") -> lis[[grp]]
# #   arrange(p) %>% 
#   ungroup() %>% 
#   group_by(Model2) %>% 
#   rstatix::adjust_pvalue(method = "fdr",p.col = "p", output.col = "p.adj")  -> p_tmp
}
# 
...
lis %>% 
  ...
  arrange(p) %>% 
  group_by(Model2, Reactor_Treatment_Dose, variable) %>% 
  select(-".y.", -p.adj, -p.format, -p.signif) %>% 
  rstatix::adjust_pvalue(method = "fdr",p.col = "p", output.col = "p.adj") %>% 
  rstatix::add_significance(p.col = "p.adj")   -> tmp_df

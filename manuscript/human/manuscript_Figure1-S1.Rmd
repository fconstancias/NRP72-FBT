---
title: " Fig. 1 - S1 - humanen manuscript "
...
# scale_fill_viridis_c(breaks = c(0,  0.01, s1, 10, 25, 50, 75, 100),
# labels = c(0,  0.01, 1, 10, 25,  50, 75, 100),
# trans = scales::pseudo_log_trans(sigma = 0.9),
# na.value = 'transparent') -> pre_speces
...
pre_plots_grp %>%
  ggsave(file="~/Desktop/16S-human_Fig1b.svg", plot=., width=8, height=10)
...
ggpubr::ggarrange(pre_Family + ylab(NULL) + theme(
                    legend.position = "null",
                    axis.title.x=element_blank(),
                    axis.text.x=element_blank(),
                    axis.ticks.x=element_blank()),
...
pre_plots %>% 
  ggsave(file="~/Desktop/16S-human_Fig1.svg", plot=., width=10, height=8)
...
plot_tmp_noleg %>% 
  ggsave(file="~/Desktop/16S-human_FigS1.svg", plot=., width=2, height=3)
...
otu %>%
  ...
  write_tsv(paste0("~/Desktop/",taxrank,"_human_donors.tsv"))

# 
ps_tmp %>%
  sample_data() %>%
  data.frame() %>%
    write_tsv(paste0("~/Desktop/metadata_human.tsv"))

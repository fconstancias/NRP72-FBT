...
ps_filt %>% 
  transform_sample_counts(function(x) x/sum(x) * 100) %>% 
  speedyseq::psmelt() %>% 
  select(OTU, Abundance, Model2_reactor_treatment_dose ,Day_of_Treatment) %>% 
...
#   group_by(Model2, OTU, Model2_reactor_treatment_dose) %>% 
#   # do(model = tseries::adf.test(.))
#   # filter(OTU == "[Eubacterium] brachy group" & Reactor_Treatment_Dose == "TR7_VAN+CCUG5916890") %>%
#   # pull(Abundance) %>%
...
#   summarise(pval_ur.df2_none =  ifelse(n() <= 5,NA,coef(attributes(ur.df(y = Abundance, type = "none", selectlags = "AIC"))$testreg)[1, 4]),
#             # pval_ur.df2_drift = ifelse(n() <= 8,NA,coef(attributes(ur.df(y = Abundance, type = "drift", selectlags = "AIC"))$testreg)[1, 4]),
#             pval_ur.df2_trend = ifelse(n() <= 5,NA,coef(attributes(ur.df(y = Abundance, type = "trend", selectlags = "AIC"))$testreg)[1, 4]),
#             pval_adf.test = tseries::adf.test(Abundance, alternative = "stationary")$p.value,
#             pval_kpss_type1 = ifelse(n() <= 5,NA, aTSA::kpss.test(Abundance, lag.short = TRUE, output =FALSE) %>%  data.frame() %>%  rownames_to_column("type") %>% filter(type == "type 1") %>% pull(p.value)),
#             pval_kpss_type2 = ifelse(n() <= 5,NA, aTSA::kpss.test(Abundance, lag.short = TRUE, output =FALSE) %>%  data.frame() %>%  rownames_to_column("type") %>% filter(type == "type 2") %>% pull(p.value)),
#             pval_kpss_type3 = ifelse(n() <= 5,NA, aTSA::kpss.test(Abundance, lag.short = TRUE, output =FALSE) %>%  data.frame() %>%  rownames_to_column("type") %>% filter(type == "type 3") %>% pull(p.value)),
#             pval_Box.test = ifelse(n() <= 5,NA, stats::Box.test(Abundance, lag = 1, type = "Ljung")$p.value)
...
classif %>% 
  ...
  left_join(map_class %>%  rename("cat_lm" = "category"),
            by = c("Model2_reactor_treatment_dose", "OTU")) %>% 
  select(Model2_reactor_treatment_dose, OTU, cat_period, cat_lm, pos:p1_neg) -> tmp
```
...
ps_filt %>% 
  ...
  ggplot(data = ., aes_string(color = "OTU", fill = "OTU")) +
  geom_area(aes_string(x = "Day_of_Treatment", y = "Abundance")) + 
  facet_grid(category ~  Model2 + Model2_reactor_treatment_dose, scales = "free") + 
...
ps_filt %>%
  ...
  # Reactor_Treatment_Dose %in% ("TR4_CCUG59168")) %>% 
  arrange(Model2_reactor_treatment_dose) %>% 
  ggplot(data = ., aes_string(x = "period", y = "Abundance", color = "category", fill = "category")) +
  geom_bar(position="fill", stat="identity") + 
  # geom_area(aes_string(x = "Day_of_Treatment", y = "Abundance")) + 
  facet_grid(Model2 ~ Treatment, scales = "free", drop = TRUE, space = "free") + 
  theme(legend.position = "bottom") -> p_cat
...
ps_filt %>%
  ...
  speedyseq::psmelt() %>%
  # filter(Reactor_Treatment_Dose %in% c("TR6_VAN90","TR4_CCUG59168", "TR5_HV292.1", "TR1_CTX+HV292.120")) %>% 
  select(OTU, Abundance, Day_of_Treatment, Model2 ,Model2_reactor_treatment_dose, Treatment, Genus) %>% 
  left_join(classif,
            by = c("Model2_reactor_treatment_dose", "OTU")) %>% 
  left_join(day_period) %>%   # filter(category %in% c("normal"),
  # filter(category %in% c("normal"),
  # Reactor_Treatment_Dose %in% ("TR4_CCUG59168")) %>% 
  arrange(Model2_reactor_treatment_dose) %>% 
  ggplot(data = ., aes_string(x = "interaction(period, category)", y = "Abundance", color = "Genus", fill = "Genus")) +
  geom_bar(position="fill", stat="identity") + 
  # geom_area(aes_string(x = "Day_of_Treatment", y = "Abundance")) + 
  facet_grid(Model2 ~ Treatment , scales = "free", drop = TRUE, space = "free") + 
  theme(legend.position = "none") + ggpubr::rotate_x_text(90) -> p_cat
...
ps_filt2 %>% 
  # microbiome::transform("compositional") %>%
  # speedyseq::psmelt() %>%
  filter(OTU %in% c("Proteus", "Eisenbergiella", "Intestinimonas") &
           Reactor_Treatment_Dose %in% c("TR2_CTX20", "TR3_CTX20", "TR2_CTX+HV292.120")) %>%
  # arrange(Reactor_Treatment_Dose %>% c("TR3_CTX20", "TR5_VAN+CCUG5916890") ) %>% 
  ggplot(data = ., aes_string(color = "OTU", fill = "OTU")) +
  geom_area(aes_string(x = "Day_of_Treatment", y = "Abundance")) + 
  facet_grid(Reactor_Treatment_Dose ~  OTU, scales = "free") + 
...
```{r}
tmp %>% 
  filter(Reactor_Treatment_Dose %in% c("TR2_CTX20"),
         OTU %in% c("Erysipelatoclostridium","Negativibacillus",
                    "CAG-56", 
                    "Lachnoclostridium",
  ...
  ggplot(data = ., aes_string(color = "cat_period", fill = "pval.adj")) +
  geom_area(aes_string(x = "Day_of_Treatment", y = "Abundance")) + 
  facet_grid(cat_lm ~  OTU, scales = "free") + 
  theme(legend.position = "bottom") -> p_cat
...
ps_filt2 %>% 
  ...
  mutate(cum_avg_change_pos = sum(Abundance[Abundance>mean(nth(Abundance, n = 3))]),
         cum_avg_change_neg = sum(Abundance[Abundance<mean(nth(Abundance,  n = 3))])) %>%
...
t_otu %>% 
  Hmisc::rcorr(type = "spearman") -> cor_tax

#######------------------

# rows.cor <- cor((t_otu),  method = "spearman")

hclust.row <- hclust(as.dist(1-cor_tax$r),"ward.D2" )
...
# it finds to be optimal, set it to search for
# at least 1 model and up 20.
d_clust <- Mclust(as.matrix(cor_tax$r), G=1:10)
m.best <- dim(d_clust$z)[2]
cat("model-based optimal number of clusters:", m.best, "\n")
...
for (cluster_nb in taxa_gpes$cluster %>%  unique()){
  ...
  if(tax %>%  length() > 1 )
  {
    ps_filt %>% 
      microbiome::transform("compositional") %>% 
      # prune_samples(get_variable(ps_filt, var) %in% group,
      # .)  %>% 
      # filter_taxa(function(x) sum(x > 0) > 0, TRUE) %>% 
      # microbiome::transform(., "compositional") %>% 
      subset_taxa(.,  eval(as.name("Genus")) %in% tax) %>% 
      speedyseq::psmelt() %>% 
      ggplot(data = ., aes_string(color = "Genus", fill = "Genus")) +
      geom_area(aes_string(x = "Day_of_Treatment", y = "Abundance")) + 
      facet_grid(Reactor_Treatment_Dose ~  ., scales = "free") + 
      theme(legend.position = "bottom")  -> out[[cluster_nb]]
    # 
    # # subset_taxa( Genus %in% tax) %>% 
    # # subset_taxa(Genus %in% c("Proteus", "Ligilactobacillus")) %>% 
    # microbiomeutilities::plot_area(., xvar="Day_of_Treatment", 
...
2) a species with a relative abundance ≥ 0.1% in baseline and increased by at least 10-fold in the relative abundance in T2 or T3.
...
in https://genomebiology.biomedcentral.com/articles/10.1186/gb-2014-15-7-r89#Sec7

This method (‘adfuller’) was run on time series mean-centered in log10-space and was called with regression parameters of no constant and no trend (regression = ‘nc’). The number of lags was chosen using the t-statistic (autolag = ‘t-stat’)
...
ps_filt %>% 
  ...
  filter(Day_of_Treatment < 15) %>% 
  group_by(OTU, Reactor_Treatment_Dose) %>% 
  # filter(OTU == "Enterococcus" & Reactor_Treatment_Dose == "CR_UNTREATED") %>%
  # arrange(Day_of_Treatment) %>% 
  # pull(Abundance) %>% 
  # do(model = tseries::adf.test(.))
  summarise(pval = ifelse(n() <= 4,NA, adf.test(Abundance)$p.value)) %>%
  # summarise(pval = ifelse(n() <= 6,NA, CADFtest(Abundance, type="trend", criterion = "AIC")$p.value)) %>% 
  # group_by(Reactor_Treatment_Dose) %>% 
  rstatix::adjust_pvalue(p.col = "pval", method = "fdr") %>% 
  filter(pval.adj >= 0.05) %>% 
  arrange(Reactor_Treatment_Dose) -> adf_test

adf_test %>% 
  group_by(Reactor_Treatment_Dose) %>% 
  summarise(n_ns = length(OTU))

# %>% 
#   unlist() %>% 
#   data.frame() -> out
# CADFtest::CADFtest(. ,type = "none", criterion = "AIC")
```
https://stackoverflow.com/questions/32200394/r-apply-adf-test-by-group 

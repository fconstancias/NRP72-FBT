...
ps_CARD %>% 
  sample_data() %>% 
  data.frame() %>% 
  select(-input:-index2) -> sample_data(ps_CARD)
```
...
"data/processed/ps_silva_dada2_human_chicken_meta.RDS" %>% 
  ...
  subset_samples(Day_from_Inoculum >= 30) %>% 
  subset_samples(Experiment != "Batch") %>% 
  subset_samples(is.na(Paul)) %>% 
  subset_samples(Day_of_Treatment >= -4) %>% 
...
ps_CARD %>% 
  ...
  phyloseq_alphas(phylo = FALSE) -> alpha_Best_Hit_ARO
```

```{r,warning= FALSE, message= FALSE}
alpha_Best_Hit_ARO %>% 
  plot_alpha_div_NRP72() -> p_a_AMRgn
...
```{r, message= FALSE}
ps_16S %>% 
  rarefy_even_depth(sample.size = 5000, rngseed = 123) %>% 
  phyloseq_alphas(phylo = FALSE) -> alpha_16S
```

```{r,warning= FALSE, message= FALSE}
alpha_16S %>% 
  plot_alpha_div_NRP72(path_group = "interaction(Fermentation, Reactor_Treatment)", ylab = "Bacterial community alpha-diversity") -> p_a_16S

p_a_16S + geom_smooth(alpha = 0.05, linetype=0, level = 0.90)
```

+ boxplot phases pour chicken?
...
ps_CARD %>% 
  ...
  plot_beta_NRP72(path_group = "interaction(Model, Fermentation, Reactor_Treatment)") -> p_b_chick_AMRgn
...
ps_CARD %>% 
  ...
  plot_beta_NRP72(path_group = "interaction(Model, Fermentation, Reactor_Treatment)") -> p_b_hum_AMRgn

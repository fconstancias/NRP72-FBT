...
```{r}
# TPM
chicken$orfs$table %>% as.data.frame() %>% select(starts_with("TPM")) -> chicken_otu_tpm

colnames(chicken_otu_tpm) <- colnames(chicken_otu_tpm) %>% str_remove_all("TPM ")
rownames(chicken_otu_tpm) <- paste0("c_", rownames(chicken_otu_tpm))

# Coverage
chicken$orfs$table %>% as.data.frame() %>% select(starts_with("Coverage")) -> chicken_otu_cov

colnames(chicken_otu_cov) <- colnames(chicken_otu_cov) %>% str_remove_all("Coverage ")
rownames(chicken_otu_cov) <- paste0("c_", rownames(chicken_otu_cov))

# Raw reads
chicken$orfs$table %>% as.data.frame() %>% select(starts_with("Raw read count")) -> chicken_otu_rr

colnames(chicken_otu_rr) <- colnames(chicken_otu_rr) %>% str_remove_all("Raw read count ")
...
chicken_physeq_tpm %>% saveRDS("~/Documents/MSc_Thesis-NRP72/data/chicken_full_tpm_orf.rds")
...
chicken_physeq_cov %>% saveRDS("~/Documents/MSc_Thesis-NRP72/data/chicken_full_cov_orf.rds")
...
chicken_physeq_rr %>% saveRDS("~/Documents/MSc_Thesis-NRP72/data/chicken_full_rr_orf.rds")
```
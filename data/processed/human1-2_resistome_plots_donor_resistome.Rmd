...
"data/processed/chicken1-2_human1-2-full_gene_catalog_phyloseq.RDS" %>% 
  here::here() %>% 
  readRDS() -> ps
...
subset_samples(ps1, sample_names(ps1) %!in% c("D3", "D19")) -> ps2
...
ps_AMR %>% 
  ...
  microViz::tax_mutate(AMR_sub_class_multi = ifelse(grepl(";", AMR_sub_class), "multidrug", AMR_sub_class)) -> ps_AMR #%>%
...
ps_inoc_AMR %>% 
  ...
  facet_wrap(Model2 ~ Antibiotic + Antibiotic_mg.L + Treatment2, scales = "free_y", ncol = 6, nrow = 4) +
  # scale_y_log10() +
...
ps_AMR_all %>% 
  ...
  ylab("Proportion - %") +
  scale_y_continuous(labels=percent) + 
  # facet_grid(Model2 ~ Antibiotic + Antibiotic_mg.L + Treatment2, scales = "free", space = "free") -> p2
  facet_wrap(Model2 ~ Antibiotic + Antibiotic_mg.L + Treatment2, scales = "free", ncol = 7, nrow = 3) -> p2
...
```{r}
p2 + facet_null() + facet_grid(Model2 ~ Antibiotic + Antibiotic_mg.L + Treatment2, scales = "free", space = "free") 
```
...
```{r}
plot_alpha_div_NRP72 <- function(df = resistome_alpha, x = "Day_of_Treatment", y = "value",  point_size = 2.5, color = "Treatment", fill  = "Treatment", shape = "Antibiotic", alpha = "Antibiotic_mg.L", facet_formula = paste0("alphadiversiy ~  Model2 "), ylab = "Resistome alpha-diversity", xlab = "Days (Treatment)", measures = c("observed", "diversity_shannon"), path_group = "interaction(Model, Reactor_Treatment_Dose)"){

...
```{r, eval=FALSE}
cd /Users/fconstan/Projects/ETH/Alessia/16S 

conda activate metabaRpipe 

cd /Users/fconstan/Projects/ETH/Alessia/16S/2

Rscript ~/Documents/GitHub/metabaRpipe/Rscripts/dada2_metabarcoding_pipeline.Rscript -i raw/ -T 4 --preset V4-1PCR --db ~/db/DADA2/silva_nr99_v138.1_train_set.fa.gz --db_species ~/db/DADA2/silva_species_assignment_v138.1.fa.gz --metadata /Users/fconstan/Documents/GitHub/NRP72-FBT/data/raw/05.10.2022_metadata_all_merged.xlsx --priors ~/Documents/GitHub/metabaRpipe/databases/priors/hv292_CGU_V4.fasta --pool FALSE

cd /Users/fconstan/Projects/ETH/Alessia/16S/1
...
outersect(
  sample_names(ps_all),
  "/Users/fconstan/Documents/GitHub/NRP72-FBT/data/raw/05.10.2022_metadata_all_merged.xlsx" %>%
    readxl::read_xlsx() %>% 
    pull(sample_name)
...
```{r}
 "/Users/fconstan/Documents/GitHub/NRP72-FBT/data/raw/05.10.2022_metadata_all_merged.xlsx" %>%
    readxl::read_xlsx() %>% 
  filter(sample_name %in% as.vector(missing_samples)) %>% 
  select(metagenomic_sample_name, everything()) %>% 
  DT::datatable()
```
...
ps_all %>% 
  physeq_add_metadata(physeq = .,
                      metadata = ( "/Users/fconstan/Documents/GitHub/NRP72-FBT/data/raw/05.10.2022_metadata_all_merged.xlsx") %>%
                        readxl::read_excel(sheet = 1), sample_column = "sample_name") %>% 
    filter_taxa(function(x) sum(x > 0) > 0, TRUE) -> ps_all
...
ps_all %>% 
  ...
  mutate(Treatment2 = factor(Treatment2, levels = c("DONOR", "UNTREATED", "AB+E. coli", "AB", "E. coli", "AB+E. faecium", "E. faecium")),
         ...
         Day_of_Treatment = as.double(Day_of_Treatment),
         Antibiotic_mg.L = ifelse(Phase == "Stab", NA, Antibiotic_mg.L),
         Antibiotic_mg.L = factor(Antibiotic_mg.L, levels = c(NA, 20, 200,90, 600)), 
         reatment = factor(Treatment, levels = c("DONOR","UNTREATED","CTX","CTX+HV292.1","HV292.1","VAN","VAN+CCUG59168", "CCUG59168"
  ))
...
9 ASV assigned to mitochondria or choloroplasts.
...
Remove some mislabeled samples:

```{r}
ps_all_clean %>% 
  subset_samples(Sample_description %!in% c("TR5-15", "TR4-1")) -> ps_all_clean
```


save without phylogeny:

```{r}
ps_all_clean %>% 
  saveRDS("/Users/fconstan/Documents/GitHub/NRP72-FBT/data/processed/16S/ps_silva_dada2_human_chicken_all_fct_no_tree_051022.RDS")
```
...
```{r}
ps_all_clean %>% 
  saveRDS("/Users/fconstan/Documents/GitHub/NRP72-FBT/data/processed/16S/ps_silva_dada2_human_chicken_all_fct_051022.RDS")
```
...
ps_all_clean %>% 
  ...
  subset_samples(Day_of_Treatment >= -4 | Experiment %in% c("Cecum", "NTC", "Mock", "HV292.1", "CCUG59168", "Continuous", "SLURRY")) %>% 
  # %    subset_samples(Model == "Human" & Day_of_Treatment <= 4) %>% 
  filter_taxa(function(x) sum(x > 0) > 0, TRUE) %>% 
    saveRDS("/Users/fconstan/Documents/GitHub/NRP72-FBT/data/processed/16S/ps_silva_dada2_human_chicken_all_fct_polyferms_051022.RDS")
```

...
```{r}
"data/processed/16S/1/ps_silva_dada2_human_chicken_meta_fact.RDS" %>% 
  here::here() %>% 
  readRDS() %>% 
...
names(treat_col) <- get_variable(phyloseq_AMR_MAG_MGE, "Treatment") %>%  
  levels()
...
```{r}
antibio_shape <- c(22, 21)
names(antibio_shape) <- sample_data(phyloseq_AMR_MAG_MGE)$Antibiotic_mg.mL %>%  levels()
```
